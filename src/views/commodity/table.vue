    ...
    <div class="page-header">
      <el-page-header @back="goBack">
        <template slot="content">
          ...
          <span>{{this.$t("commodity.commodity_table")}}</span>
        </template>
      </el-page-header>
    </div>
    <div class="page-content">
      <div class="container" style="flex-grow: 1;">
        ...
        >
          ...
          <el-table-column prop="goodsName" :label="$t('commodity.name')">
            <template slot-scope="scope">
              <el-button type="text" @click="toDetailPage(scope.row.id)">{{ scope.row.goodsName }}</el-button>
            </template>
          </el-table-column>
          <el-table-column prop="goodsStatus" :label="$t('commodity.status')" width="120">
            <template slot-scope="scope">{{ scope.row.goodsStatus|commodity_status }}</template>
          </el-table-column>
          <el-table-column prop="price" :label="$t('commodity.selling_price')"></el-table-column>
          <el-table-column prop="purchasePrice" :label="$t('commodity.purchase_price')"></el-table-column>
          <el-table-column prop="stock" :label="$t('commodity.stock')" width="120"></el-table-column>
          <el-table-column prop="packingValue" :label="$t('commodity.packing')" width="90"></el-table-column>
          <el-table-column prop="createTime" :label="$t('commodity.createTime')" width="180">
...
import CreateDialop from "./createDialop";

export default {
  components: { EditText, DeleteDialop, CreateDialop },
  data() {
    return {
      loading: false,
      tableSearchText: "",
